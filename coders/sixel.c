                  ...
                  sixel_output_t *context)
{
    ...
    component_map = (unsigned char *)AcquireMagickMemory(len);
    ...
    for (y = i = 0; y < height; y++) {
        ...
        if (++i == 6 || y == height - 1) {
            i = 0;
            /* crate color nodes for each colors */
            for (c = 0; c < ncolors; c++) {
                /* search the left end of the node */
                for (left = 0; left < width; left++) {
                    if (component_map[c * width + left] != 0) {
                        /* search the right end of the node */
                        ...
                        /* allocate or re-use node */
                        np = context->node_free;
                        if (np != NULL) {
...
%
*/
static MagickBooleanType WriteSIXELImage(const ImageInfo *image_info,Image *image,ExceptionInfo *exception)
{
  MagickBooleanType
    status;
